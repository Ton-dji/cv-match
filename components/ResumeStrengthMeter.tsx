...
export const ResumeStrengthMeter = ({ data }: ResumeStrengthMeterProps) => {
  ...
  React.useEffect(() => {
      ...
      else tempSuggestions.push(t('add_full_name') || "Add your full name");

      if (data.email) tempScore += 10;
      else tempSuggestions.push(t('add_email') || "Add your email address");

      if (data.phone) tempScore += 5;
      
      // Content
      if (data.summary && data.summary.length > 50) tempScore += 15;
      else tempSuggestions.push(t('add_summary') || "Add a professional summary (50+ characters)");

      if (data.experience && data.experience.length > 0) tempScore += 25;
      else tempSuggestions.push(t('add_experience_tip') || "Add at least one work experience");

      if (data.education && data.education.length > 0) tempScore += 15;
      else tempSuggestions.push(t('add_education_tip') || "Add your education details");

      if (data.skills && data.skills.length >= 3) tempScore += 15;
      else tempSuggestions.push(t('add_skills_tip') || "Add at least 3 key skills");
    ...
    <Card className="mb-6">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-medium flex justify-between">
            {t('resume_strength') || "Resume Strength"}
      ...
      <CardContent>
        ...
        {suggestions.length > 0 && (
            <div className="space-y-2">
                <p className="text-xs text-muted-foreground font-semibold">{t('improve_score') || "Improve your score:"}</p>
        ...
        {score === 100 && (
             <div className="flex items-center gap-2 text-xs text-green-600 font-medium">
                ...
                <span>{t('profile_optimized') || "Your profile is optimized!"}</span>
            </div>
        )}
      </CardContent>
    </Card>
